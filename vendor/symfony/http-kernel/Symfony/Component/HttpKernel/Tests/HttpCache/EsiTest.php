...
class EsiTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAddSurrogateControl()
    {
        ...
        $response = new Response('foo <esi:includes src="" />');
    ...
    public function testRenderIncludeTag()
    {
        ...
        $this->assertEquals('<esi:includes src="/" onerror="continue" alt="/alt" />', $esi->renderIncludeTag('/', '/alt', true));
        $this->assertEquals('<esi:includes src="/" alt="/alt" />', $esi->renderIncludeTag('/', '/alt', false));
        $this->assertEquals('<esi:includes src="/" onerror="continue" />', $esi->renderIncludeTag('/'));
        $this->assertEquals('<esi:comment text="some comment" />'."\n".'<esi:includes src="/" onerror="continue" alt="/alt" />', $esi->renderIncludeTag('/', '/alt', true, 'some comment'));
    }
    ...
    public function testProcess()
    {
        ...
        $request = Request::create('/');
        $response = new Response('foo <esi:comment text="some comment" /><esi:includes src="..." alt="alt" onerror="continue" />');
        $esi->process($request, $response);

        $this->assertEquals('foo <?php echo $this->esi->handle($this, \'...\', \'alt\', true) ?>'."\n", $response->getContent());
        $this->assertEquals('ESI', $response->headers->get('x-body-eval'));

        $response = new Response('foo <esi:comment text="some comment" /><esi:includes src="foo\'" alt="bar\'" onerror="continue" />');
        $esi->process($request, $response);
        ...
        $response = new Response('foo <esi:includes src="..."></esi:includes>');
        $esi->process($request, $response);
    ...
    public function testProcessWhenNoSrcInAnEsi()
    {
        ...
        $request = Request::create('/');
        $response = new Response('foo <esi:includes />');
        $esi->process($request, $response);
    }
