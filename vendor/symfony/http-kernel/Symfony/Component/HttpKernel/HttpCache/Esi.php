...
class Esi
{
    ...
    {
        if (false !== strpos($response->getContent(), '<esi:includes')) {
     ...
     * @param string  $comment      A comment to add as an esi:includes tag
     *
     * @return string
     */
    public function renderIncludeTag($uri, $alt = null, $ignoreErrors = true, $comment = '')
    {
        $html = sprintf('<esi:includes src="%s"%s%s />',
            $uri,
    ...
    public function process(Request $request, Response $response)
    {
        ...
        $content = preg_replace_callback('#<esi\:includes\s+(.*?)\s*(?:/|</esi\:includes)>#', array($this, 'handleEsiIncludeTag'), $content);
    ...
    /**
     * Handles an ESI includes tag (called internally).
     *
     * @param array $attributes An array containing the attributes.
     *
     * @return string The response content for the includes.
     *
     * @throws \RuntimeException
     */
